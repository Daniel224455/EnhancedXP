    ...
    )
{
	...
	#if defined(_WIN64)
    if ((*((LPBYTE)lpContext->Rip) == 0xFA) ||
        (*((LPBYTE)lpContext->Rip) == 0xFB))
    {
        bFixBlt = TRUE;
        lpContext->Rip++;
        lRet = EXCEPTION_CONTINUE_EXECUTION;
    }
    else if ((*((LPBYTE)lpContext->Rip) == 0xF7) ||     // Handle idiv
             (*((LPBYTE)lpContext->Rip+1) == 0xF7))     // Handle 16 bit idiv
    {
        DPFN( eDbgLevelWarning, "Detected 'idiv' overflow: validating rdx:rax");
        lpContext->Rdx=0;
        if ((LONG)lpContext->Rax < 0)
        {
            lpContext->Rax = (DWORD)(-(LONG)lpContext->Rax);
        }
        lRet = EXCEPTION_CONTINUE_EXECUTION;
    }
	#else
