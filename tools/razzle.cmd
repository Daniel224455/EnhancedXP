...
@rem Set a username for your build
set __username="%username%"
@rem If you're an authorized Git dev, then your Git username will be username instead
for /f "delims=" %%a in ('git config user.name') do set "__username=%%a"
set "__username=%__username: =%"
set "__username_as_shortname=!__username:~0,32!"
...
if /I "%1" == "OfficialBuild" set _ArgOfficial=true&goto ArgOK
...
@rem Previous line doesn't work on NT 4.0 and older. Some ancient build machines still use NT 4.0
...
@rem set STAMPINF_DATE=10/01/2002
...
@rem despite that we use Git instead
@rem
...
if "%_ArgOffline%" == "true" (
   ...
   SET SIGNTOOL_SIGN=/f "%RazzleToolPath%\driver.pfx" /d "Microsoft Windows OpenXP" /du "http://ntbld"
) else (
   @rem Online: Use autoenrolled signing cert in cert store (only available to internal Microsoft users)
   SET SIGNTOOL_SIGN=/a /uw /r "Microsoft Test Root Authority" /d "Microsoft Windows OpenXP" /du "http://ntbld"
)
...
if "%_ArgOffline%" == "true" (
    set _BuildBranch=vbl_feat_prerelease
) else (
	@rem We can fetch current branch via `git branch --show-current`,
	@rem but this is available since 2.22 and won't work for XP/2003 (max Git for them is 2.10)

    for /f %%i in ('git rev-parse --abbrev-ref HEAD') do set _BuildBranch=%%i
)
...
@rem for testing purposes only, in not only LAB03_N and LAB03_DEV
@rem
...
@rem but without useless checks, because it is moot actually...
@rem
if "%_ArgOfficial%" == "true" (
	set OFFICIAL_BUILD_MACHINE=%COMPUTERNAME%
	set _BuildNotifyDL=ntblus
